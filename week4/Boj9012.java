...
public class Boj9012 {
  ...
  public Boj9012(int capacity) { // 생성자. 매개변수로 스택의 용량을 줌
  ...
  // 스택에 x를 push
  public char push(char x) throws Boj9012.OverflowIntStackException {
    if (ptr >= max) // 스택이 가득차있으면
      throw new Boj9012.OverflowIntStackException();
    return stk[ptr++] = x;
  }

  // 스택에서 데이터를 pop -> 데이터를 꺼내서 버림
  public char pop() throws Boj9012.EmptyInStackException {
    if(ptr <= 0) // 스택이 빈 경우 pop할 데이터가 없으므로
      throw new Boj9012.EmptyInStackException();
    return stk[ptr--];
  }

  // 스택에서 데이터를 peak -> 데이터를 꺼내서 확인
  public char peak() throws Boj9012.EmptyInStackException {
    if(ptr <= 0) {
      throw new Boj9012.EmptyInStackException();
    }
    return stk[ptr -1];
